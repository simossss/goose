...
assert_file_contains "$readiness_strict_pass_output" "Health Connect audit log is included in the evidence byte/hash manifest." "PR readiness strict"
...
assert_file_contains "$SCRIPT_DIR/android_final_pr_gate.sh" "Final PR gate requires a clean git worktree." "final PR gate"
assert_file_contains "$SCRIPT_DIR/android_final_pr_gate.sh" "requires HEAD to match the configured upstream" "final PR gate"
assert_file_contains "$SCRIPT_DIR/android_final_pr_gate.sh" "Final PR gate requires HEAD (\$head_commit) to match upstream" "final PR gate"
assert_file_contains "$SCRIPT_DIR/android_final_pr_gate.sh" "Pull/rebase or push the current commit first" "final PR gate"
assert_file_contains "$SCRIPT_DIR/android_partial_phone_gate.sh" "Partial phone gate requires a clean git worktree." "partial phone gate"
...
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "BLE session audit log must be included in the evidence byte/hash manifest." "PR readiness"
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "Health Connect audit log must be included in the evidence byte/hash manifest." "PR readiness"
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "Step-validation audit log must be included in the evidence byte/hash manifest." "PR readiness"
...
assert_file_contains "$APP_DIR/README.md" 'requires a clean git worktree whose `HEAD` is present on the configured upstream' "root README"
...
assert_file_contains "$ANDROID_DIR/README.md" 'requires a clean git worktree whose `HEAD` is present on the configured upstream' "Android README"
