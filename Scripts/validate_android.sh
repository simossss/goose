...
readiness_capture_counts_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-capture-counts-strict.XXXXXX")"
readiness_ble_counts_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-ble-counts-strict.XXXXXX")"
readiness_ble_command_ready_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-ble-command-ready-strict.XXXXXX")"
readiness_health_attempt_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-health-attempt-strict.XXXXXX")"
readiness_health_sdk_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-health-sdk-strict.XXXXXX")"
readiness_health_counts_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-health-counts-strict.XXXXXX")"
...
readiness_duplicate_manifest_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-duplicate-manifest-strict.XXXXXX")"
...
readiness_step_counts_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-step-counts-strict.XXXXXX")"
readiness_step_required_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-readiness-step-required-strict.XXXXXX")"
...
readiness_device_serial_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-device-serial-strict.XXXXXX")"
readiness_device_kind_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-device-kind-strict.XXXXXX")"
readiness_device_kind_derived_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-device-kind-derived-strict.XXXXXX")"
readiness_device_identity_strict_output="$(mktemp "${TMPDIR:-/tmp}/goose-android-device-identity-strict.XXXXXX")"
...
TMP_FILES+=("$checklist_output" "$partial_checklist_output" "$readiness_output" "$readiness_strict_output" "$readiness_strict_pass_output" "$readiness_partial_output" "$readiness_partial_strict_output" "$readiness_crash_strict_output" "$readiness_logcat_marker_strict_output" "$readiness_stale_capture_marker_strict_output" "$readiness_package_strict_output" "$readiness_capture_counts_strict_output" "$readiness_ble_counts_strict_output" "$readiness_ble_command_ready_strict_output" "$readiness_health_attempt_strict_output" "$readiness_health_sdk_strict_output" "$readiness_health_counts_strict_output" "$readiness_health_success_strict_output" "$readiness_manifest_strict_output" "$readiness_stale_manifest_strict_output" "$readiness_incomplete_manifest_strict_output" "$readiness_duplicate_manifest_strict_output" "$readiness_audit_manifest_strict_output" "$readiness_health_audit_manifest_strict_output" "$readiness_step_counts_strict_output" "$readiness_step_required_strict_output" "$readiness_step_audit_manifest_strict_output" "$readiness_rotated_audit_manifest_strict_output" "$readiness_stale_commit_strict_output" "$readiness_stale_debug_apk_strict_output" "$readiness_dirty_status_strict_output" "$readiness_device_serial_strict_output" "$readiness_device_kind_strict_output" "$readiness_device_kind_derived_strict_output" "$readiness_device_identity_strict_output" "$readiness_android_version_strict_output" "$readiness_adb_devices_strict_output" "$readiness_final_adb_state_strict_output" "$readiness_database_artifact_strict_output" "$readiness_evidence_result_fail_strict_output" "$readiness_pull_result_fail_strict_output" "$readiness_collect_error_strict_output" "$final_gate_dry_run_output" "$partial_gate_dry_run_output" "$inspect_session_detail_output" "$synthetic_session_db")
...
synthetic_capture_counts_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-capture-counts-evidence.XXXXXX")"
synthetic_ble_counts_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-ble-counts-evidence.XXXXXX")"
synthetic_ble_command_ready_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-ble-command-ready-evidence.XXXXXX")"
synthetic_health_attempt_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-health-attempt-evidence.XXXXXX")"
synthetic_health_sdk_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-health-sdk-evidence.XXXXXX")"
synthetic_health_counts_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-health-counts-evidence.XXXXXX")"
...
synthetic_duplicate_manifest_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-duplicate-manifest-evidence.XXXXXX")"
...
synthetic_step_counts_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-step-counts-evidence.XXXXXX")"
synthetic_step_required_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-step-required-evidence.XXXXXX")"
...
synthetic_device_serial_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-device-serial-evidence.XXXXXX")"
synthetic_device_kind_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-device-kind-evidence.XXXXXX")"
synthetic_device_kind_derived_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-device-kind-derived-evidence.XXXXXX")"
synthetic_device_identity_evidence_dir="$(mktemp -d "${TMPDIR:-/tmp}/goose-android-device-identity-evidence.XXXXXX")"
...
TMP_DIRS+=("$synthetic_evidence_dir" "$synthetic_partial_evidence_dir" "$synthetic_crash_evidence_dir" "$synthetic_logcat_marker_evidence_dir" "$synthetic_stale_capture_marker_evidence_dir" "$synthetic_package_evidence_dir" "$synthetic_capture_counts_evidence_dir" "$synthetic_ble_counts_evidence_dir" "$synthetic_ble_command_ready_evidence_dir" "$synthetic_health_attempt_evidence_dir" "$synthetic_health_sdk_evidence_dir" "$synthetic_health_counts_evidence_dir" "$synthetic_health_success_evidence_dir" "$synthetic_manifest_evidence_dir" "$synthetic_stale_manifest_evidence_dir" "$synthetic_incomplete_manifest_evidence_dir" "$synthetic_duplicate_manifest_evidence_dir" "$synthetic_audit_manifest_evidence_dir" "$synthetic_health_audit_manifest_evidence_dir" "$synthetic_step_counts_evidence_dir" "$synthetic_step_required_evidence_dir" "$synthetic_step_audit_manifest_evidence_dir" "$synthetic_rotated_audit_manifest_evidence_dir" "$synthetic_stale_commit_evidence_dir" "$synthetic_stale_debug_apk_evidence_dir" "$synthetic_dirty_status_evidence_dir" "$synthetic_device_serial_evidence_dir" "$synthetic_device_kind_evidence_dir" "$synthetic_device_kind_derived_evidence_dir" "$synthetic_device_identity_evidence_dir" "$synthetic_android_version_evidence_dir" "$synthetic_adb_devices_evidence_dir" "$synthetic_final_adb_state_evidence_dir" "$synthetic_database_artifact_evidence_dir" "$synthetic_evidence_result_fail_evidence_dir" "$synthetic_pull_result_fail_evidence_dir" "$synthetic_collect_error_evidence_dir")
...
cp "$synthetic_evidence_dir"/* "$synthetic_device_serial_evidence_dir"/
printf 'different-physical-android-smoke\n' > "$synthetic_device_serial_evidence_dir/android-serial.txt"
write_synthetic_manifest "$synthetic_device_serial_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_device_serial_evidence_dir" > "$readiness_device_serial_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched device serial evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_device_kind_evidence_dir"/
printf 'emulator\n' > "$synthetic_device_kind_evidence_dir/android-device-kind.txt"
write_synthetic_manifest "$synthetic_device_kind_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_device_kind_evidence_dir" > "$readiness_device_kind_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched device kind evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_device_kind_derived_evidence_dir"/
printf 'emulator-5554\n' > "$synthetic_device_kind_derived_evidence_dir/android-serial.txt"
printf 'Google\n' > "$synthetic_device_kind_derived_evidence_dir/device-manufacturer.txt"
printf 'sdk_gphone64_arm64\n' > "$synthetic_device_kind_derived_evidence_dir/device-model.txt"
write_synthetic_manifest "$synthetic_device_kind_derived_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_device_kind_derived_evidence_dir" > "$readiness_device_kind_derived_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with emulator-derived device evidence labeled physical" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_device_identity_evidence_dir"/
printf 'Other\n' > "$synthetic_device_identity_evidence_dir/device-manufacturer.txt"
printf 'Device\n' > "$synthetic_device_identity_evidence_dir/device-model.txt"
write_synthetic_manifest "$synthetic_device_identity_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_device_identity_evidence_dir" > "$readiness_device_identity_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched device identity evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_android_version_evidence_dir"/
printf '15\n' > "$synthetic_android_version_evidence_dir/android-version.txt"
write_synthetic_manifest "$synthetic_android_version_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_android_version_evidence_dir" > "$readiness_android_version_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched Android version evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_adb_devices_evidence_dir"/
printf 'List of devices attached\nphysical-android-smoke\toffline\n' > "$synthetic_adb_devices_evidence_dir/adb-devices.txt"
write_synthetic_manifest "$synthetic_adb_devices_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_adb_devices_evidence_dir" > "$readiness_adb_devices_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with offline adb devices evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_final_adb_state_evidence_dir"/
printf 'offline\n' > "$synthetic_final_adb_state_evidence_dir/adb-state-final.txt"
...
printf 'RESULT: FAIL\n' > "$synthetic_evidence_result_fail_evidence_dir/evidence-result.txt"
write_synthetic_manifest "$synthetic_evidence_result_fail_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_evidence_result_fail_evidence_dir" > "$readiness_evidence_result_fail_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with a failed evidence-result.txt" >&2
  exit 1
fi
cp "$synthetic_evidence_dir"/* "$synthetic_pull_result_fail_evidence_dir"/
printf 'RESULT: FAIL\n' > "$synthetic_pull_result_fail_evidence_dir/pull-android-database-result.txt"
write_synthetic_manifest "$synthetic_pull_result_fail_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_pull_result_fail_evidence_dir" > "$readiness_pull_result_fail_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with a failed pull-android-database-result.txt" >&2
  exit 1
fi
...
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_step_counts_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_step_counts_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_step_counts_evidence_dir"
step_counts_summary_tmp="$synthetic_step_counts_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "- Passed events: 1" && !replaced {
    print "- Passed events: 0"
    replaced = 1
    next
  }
  { print }
' "$synthetic_step_counts_evidence_dir/phone-handoff-summary.md" > "$step_counts_summary_tmp"
mv "$step_counts_summary_tmp" "$synthetic_step_counts_evidence_dir/phone-handoff-summary.md"
write_synthetic_manifest "$synthetic_step_counts_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_step_counts_evidence_dir" > "$readiness_step_counts_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched step-validation count evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_step_required_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_step_required_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_step_required_evidence_dir"
step_required_summary_tmp="$synthetic_step_required_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "- Selected delta events: 1" && !selected_delta_replaced {
    print "- Selected delta events: 0"
    selected_delta_replaced = 1
    next
  }
  $0 == "- Passing session selected-delta events: 1" && !passing_selected_delta_replaced {
    print "- Passing session selected-delta events: 0"
    passing_selected_delta_replaced = 1
    next
  }
  { print }
' "$synthetic_step_required_evidence_dir/phone-handoff-summary.md" > "$step_required_summary_tmp"
mv "$step_required_summary_tmp" "$synthetic_step_required_evidence_dir/phone-handoff-summary.md"
step_required_inspect_tmp="$synthetic_step_required_evidence_dir/inspect-android-capture.txt.tmp"
awk '
  $0 == "step validation selected delta events: 1" && !selected_delta_replaced {
    print "step validation selected delta events: 0"
    selected_delta_replaced = 1
    next
  }
  $0 == "step validation passing session selected-delta events: 1" && !passing_selected_delta_replaced {
    print "step validation passing session selected-delta events: 0"
    passing_selected_delta_replaced = 1
    next
  }
  { print }
' "$synthetic_step_required_evidence_dir/inspect-android-capture.txt" > "$step_required_inspect_tmp"
mv "$step_required_inspect_tmp" "$synthetic_step_required_evidence_dir/inspect-android-capture.txt"
write_synthetic_manifest "$synthetic_step_required_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_step_required_evidence_dir" > "$readiness_step_required_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with missing required step selected-delta evidence" >&2
  exit 1
fi
...
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_capture_counts_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_capture_counts_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_capture_counts_evidence_dir"
capture_counts_summary_tmp="$synthetic_capture_counts_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "- Decoded frame rows: 1" && !replaced {
    print "- Decoded frame rows: 0"
    replaced = 1
    next
  }
  { print }
' "$synthetic_capture_counts_evidence_dir/phone-handoff-summary.md" > "$capture_counts_summary_tmp"
mv "$capture_counts_summary_tmp" "$synthetic_capture_counts_evidence_dir/phone-handoff-summary.md"
write_synthetic_manifest "$synthetic_capture_counts_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_capture_counts_evidence_dir" > "$readiness_capture_counts_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched capture count evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_ble_counts_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_ble_counts_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_ble_counts_evidence_dir"
ble_counts_summary_tmp="$synthetic_ble_counts_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "- Ready events: 1" && !replaced {
    print "- Ready events: 0"
    replaced = 1
    next
  }
  { print }
' "$synthetic_ble_counts_evidence_dir/phone-handoff-summary.md" > "$ble_counts_summary_tmp"
mv "$ble_counts_summary_tmp" "$synthetic_ble_counts_evidence_dir/phone-handoff-summary.md"
write_synthetic_manifest "$synthetic_ble_counts_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_ble_counts_evidence_dir" > "$readiness_ble_counts_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with mismatched BLE session count evidence" >&2
  exit 1
fi
...
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_health_attempt_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_health_attempt_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_health_attempt_evidence_dir"
health_attempt_summary_tmp="$synthetic_health_attempt_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "- Ready write started events: 1" && !replaced {
    print "- Ready write started events: 0"
    replaced = 1
    next
  }
  { print }
' "$synthetic_health_attempt_evidence_dir/phone-handoff-summary.md" > "$health_attempt_summary_tmp"
mv "$health_attempt_summary_tmp" "$synthetic_health_attempt_evidence_dir/phone-handoff-summary.md"
health_attempt_inspect_tmp="$synthetic_health_attempt_evidence_dir/inspect-android-capture.txt.tmp"
awk '
  $0 == "health sync ready write started events: 1" && !replaced {
    print "health sync ready write started events: 0"
    replaced = 1
    next
  }
  { print }
' "$synthetic_health_attempt_evidence_dir/inspect-android-capture.txt" > "$health_attempt_inspect_tmp"
mv "$health_attempt_inspect_tmp" "$synthetic_health_attempt_evidence_dir/inspect-android-capture.txt"
write_synthetic_manifest "$synthetic_health_attempt_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_health_attempt_evidence_dir" > "$readiness_health_attempt_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with missing ready Health Connect write attempt evidence" >&2
  exit 1
fi
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_health_sdk_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_health_sdk_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_health_sdk_evidence_dir"
health_sdk_summary_tmp="$synthetic_health_sdk_evidence_dir/phone-handoff-summary.md.tmp"
awk '
  $0 == "Android: 16 (SDK 36)" {
    print "Android: 13 (SDK 33)"
    next
  }
  { print }
' "$synthetic_health_sdk_evidence_dir/phone-handoff-summary.md" > "$health_sdk_summary_tmp"
mv "$health_sdk_summary_tmp" "$synthetic_health_sdk_evidence_dir/phone-handoff-summary.md"
printf '13\n' > "$synthetic_health_sdk_evidence_dir/android-version.txt"
printf '33\n' > "$synthetic_health_sdk_evidence_dir/android-sdk.txt"
write_synthetic_manifest "$synthetic_health_sdk_evidence_dir"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_health_sdk_evidence_dir" > "$readiness_health_sdk_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with Health Connect write evidence from Android < 14" >&2
  exit 1
fi
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_health_counts_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_health_counts_evidence_dir/evidence-gates.txt"
...
awk '
  ...
  { print }
' "$synthetic_health_counts_evidence_dir/phone-handoff-summary.md" > "$health_counts_summary_tmp"
...
cp "$synthetic_evidence_dir/phone-handoff-summary.md" "$synthetic_duplicate_manifest_evidence_dir/phone-handoff-summary.md"
cp "$synthetic_evidence_dir/evidence-gates.txt" "$synthetic_duplicate_manifest_evidence_dir/evidence-gates.txt"
copy_required_evidence_artifacts "$synthetic_evidence_dir" "$synthetic_duplicate_manifest_evidence_dir"
write_synthetic_manifest "$synthetic_duplicate_manifest_evidence_dir"
sed -n '2p' "$synthetic_duplicate_manifest_evidence_dir/evidence-files-manifest.txt" >> "$synthetic_duplicate_manifest_evidence_dir/evidence-files-manifest.txt"
if "$SCRIPT_DIR/android_pr_readiness.sh" --strict "$synthetic_duplicate_manifest_evidence_dir" > "$readiness_duplicate_manifest_strict_output" 2>&1; then
  echo "PR readiness strict mode unexpectedly passed with duplicate evidence manifest paths" >&2
  exit 1
fi
...
assert_file_contains "$checklist_output" "Health Connect record-summary write started events: at least 1." "final phone checklist"
...
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "record-summary evidence from a real Android phone" "Android install handoff"
...
assert_file_contains "$partial_checklist_output" "Scripts/android_partial_phone_gate.sh tmp/android-phone-partial-gate-real --skip-validate" "partial phone checklist"
...
assert_file_contains "$partial_checklist_output" "Health Connect record-summary write started events: at least 1." "partial phone checklist"
assert_file_contains "$partial_checklist_output" "Daily local estimate metrics" "partial phone checklist"
assert_file_contains "$partial_checklist_output" "Daily device counter metrics" "partial phone checklist"
assert_file_contains "$partial_checklist_output" "Motion" "partial phone checklist"
...
assert_file_contains "$readiness_output" "Scripts/android_final_pr_gate.sh tmp/android-phone-final-gate-real --skip-validate" "PR readiness"
assert_file_contains "$readiness_output" "after \`Scripts/prepare_android_phone_evidence.sh\` and the controlled capture" "PR readiness"
...
assert_file_contains "$readiness_strict_pass_output" "Phone handoff capture counts, including daily activity provenance counts, match inspect-android-capture.txt." "PR readiness strict"
...
assert_file_contains "$readiness_strict_pass_output" "Derived device kind: physical" "PR readiness strict"
...
assert_file_contains "$readiness_strict_pass_output" "Device kind matches the serial/manufacturer/model emulator check." "PR readiness strict"
...
assert_file_contains "$readiness_strict_pass_output" "Evidence package summary versionCode=1: 1" "PR readiness strict"
...
assert_file_contains "$readiness_capture_counts_strict_output" "Phone handoff capture counts, including decoded frame counts, must match inspect-android-capture.txt." "PR readiness capture counts strict"
assert_file_contains "$readiness_capture_counts_strict_output" "Phone handoff daily activity provenance counts must be present, numeric, and match inspect-android-capture.txt." "PR readiness capture counts strict"
...
assert_file_contains "$readiness_health_attempt_strict_output" "Health Connect permission grant and real planned write attempt on Android 14+ / SDK 34+, including permissions-ready planned-write context and record-summary provenance." "PR readiness Health Connect attempt strict"
...
assert_file_contains "$readiness_health_sdk_strict_output" "Health Connect permission grant and real planned write attempt on Android 14+ / SDK 34+, including permissions-ready planned-write context and record-summary provenance." "PR readiness Health Connect SDK strict"
assert_file_contains "$readiness_health_sdk_strict_output" "Strict PR readiness: FAIL" "PR readiness Health Connect SDK strict"
...
assert_file_contains "$readiness_health_counts_strict_output" "Strict PR readiness: FAIL" "PR readiness Health Connect counts strict"
...
assert_file_contains "$readiness_duplicate_manifest_strict_output" "Evidence bundle must include a valid evidence-files-manifest.txt with matching path, byte, and SHA-256 columns for required evidence files." "PR readiness duplicate manifest strict"
assert_file_contains "$readiness_duplicate_manifest_strict_output" "Strict PR readiness: FAIL" "PR readiness duplicate manifest strict"
...
assert_file_contains "$readiness_step_counts_strict_output" "Phone handoff step-validation counts must match inspect-android-capture.txt." "PR readiness step counts strict"
assert_file_contains "$readiness_step_counts_strict_output" "Strict PR readiness: FAIL" "PR readiness step counts strict"
assert_file_contains "$readiness_step_required_strict_output" "Step-counter decoder confirmation from real counted-step evidence with \`--require-step-validation\`, capture-session binding, decoded session frames, and a nonzero selected counter delta in the same audit row." "PR readiness step required strict"
assert_file_contains "$readiness_step_required_strict_output" "Strict PR readiness: FAIL" "PR readiness step required strict"
...
assert_file_contains "$readiness_device_serial_strict_output" "Phone handoff summary device serial must match android-serial.txt." "PR readiness device serial strict"
assert_file_contains "$readiness_device_serial_strict_output" "Strict PR readiness: FAIL" "PR readiness device serial strict"
assert_file_contains "$readiness_device_kind_strict_output" "Phone handoff summary device kind must match android-device-kind.txt and the serial/manufacturer/model emulator check." "PR readiness device kind strict"
assert_file_contains "$readiness_device_kind_strict_output" "Strict PR readiness: FAIL" "PR readiness device kind strict"
assert_file_contains "$readiness_device_kind_derived_strict_output" "Phone handoff summary device kind must match android-device-kind.txt and the serial/manufacturer/model emulator check." "PR readiness derived device kind strict"
assert_file_contains "$readiness_device_kind_derived_strict_output" "Strict PR readiness: FAIL" "PR readiness derived device kind strict"
assert_file_contains "$readiness_device_identity_strict_output" "Phone handoff summary device identity must match device-manufacturer.txt and device-model.txt." "PR readiness device identity strict"
assert_file_contains "$readiness_device_identity_strict_output" "Strict PR readiness: FAIL" "PR readiness device identity strict"
assert_file_contains "$readiness_android_version_strict_output" "Phone handoff summary Android version must match android-version.txt and android-sdk.txt." "PR readiness Android version strict"
assert_file_contains "$readiness_android_version_strict_output" "Strict PR readiness: FAIL" "PR readiness Android version strict"
assert_file_contains "$readiness_adb_devices_strict_output" "adb-devices.txt must list the handoff device serial as online." "PR readiness adb devices strict"
...
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "fail_before_device_evidence" "phone evidence collector"
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "adb devices failed. Status snapshot was written" "phone evidence collector"
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "write_file_manifest" "phone evidence collector"
...
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "phone evidence collector adb timeout"
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "run_to_file_with_timeout" "phone evidence collector adb timeout"
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "Android full logcat snapshot" "phone evidence collector adb timeout"
assert_file_contains "$SCRIPT_DIR/collect_android_phone_evidence.sh" "Android installed APK read" "phone evidence collector adb timeout"
...
assert_file_contains "$SCRIPT_DIR/android_phone_final_gate.sh" "record-summary provenance: required" "final phone gate"
assert_file_contains "$SCRIPT_DIR/android_partial_phone_gate.sh" "record-summary evidence" "partial phone gate"
assert_file_contains "$SCRIPT_DIR/android_final_pr_gate.sh" "write-attempt record summary" "final PR gate"
...
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "phone evidence prep adb timeout"
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "run_with_timeout" "phone evidence prep adb timeout"
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "Android adb devices" "phone evidence prep adb timeout"
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "Android adb state" "phone evidence prep adb timeout"
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "Android evidence marker file write" "phone evidence prep adb timeout"
assert_file_contains "$SCRIPT_DIR/prepare_android_phone_evidence.sh" "Android evidence marker log write" "phone evidence prep adb timeout"
...
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "evidence-result.txt must report PASS for the capture inspection gate." "PR readiness"
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "pull-android-database-result.txt must report PASS for the database pull helper." "PR readiness"
...
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "Phone handoff summary device kind must match android-device-kind.txt and the serial/manufacturer/model emulator check." "PR readiness"
...
assert_file_contains "$SCRIPT_DIR/android_pr_readiness.sh" "Evidence package summary versionCode=1:" "PR readiness"
...
assert_file_contains "$APP_DIR/README.md" "numeric daily activity provenance counts" "root README"
assert_file_contains "$APP_DIR/README.md" "record-summary provenance" "root README"
...
assert_file_contains "$APP_DIR/README.md" "Scripts/android_partial_phone_gate.sh tmp/android-phone-partial-gate-real --skip-validate" "root README"
...
assert_file_contains "$APP_DIR/README.md" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "root README"
assert_file_contains "$APP_DIR/README.md" "GOOSE_ANDROID_INSTRUMENTATION_TIMEOUT_SECONDS" "root README"
assert_file_contains "$APP_DIR/README.md" "permissions-ready write-attempt and record-summary evidence" "root README"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "Android debug install adb timeout"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "run_with_timeout" "Android debug install adb timeout"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "Android adb devices" "Android debug install adb timeout"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "Android adb state" "Android debug install adb timeout"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "Android debug APK install" "Android debug install adb timeout"
assert_file_contains "$SCRIPT_DIR/install_android_debug.sh" "Android app launch" "Android debug install adb timeout"
...
assert_file_contains "$ANDROID_DIR/README.md" "daily activity local-estimate/device-counter provenance counts" "Android README"
assert_file_contains "$ANDROID_DIR/README.md" "record-summary provenance counts" "Android README"
...
assert_file_contains "$ANDROID_DIR/README.md" "Scripts/android_final_pr_gate.sh tmp/android-phone-final-gate-real --skip-validate" "Android README"
assert_file_contains "$ANDROID_DIR/README.md" "Scripts/android_partial_phone_gate.sh tmp/android-phone-partial-gate-real --skip-validate" "Android README"
assert_file_contains "$ANDROID_DIR/README.md" "Remaining Phone-Bound Proof" "Android README"
assert_file_contains "$ANDROID_DIR/README.md" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "Android README"
assert_file_contains "$ANDROID_DIR/README.md" "GOOSE_ANDROID_INSTRUMENTATION_TIMEOUT_SECONDS" "Android README"
...
assert_file_contains "$ANDROID_DIR/app/src/main/java/com/goose/android/GooseStoreReporter.java" "health sync record-summary write started events" "Android evidence report"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "health sync record-summary write started events" "Android evidence report"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "ble session client hello command-ready completed events:" "Android evidence report"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "ble session ready hello command-ready events:" "Android evidence report"
...
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "ready-plan audit missing write_started record_summary" "Android Health Connect audit"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "fileHasLineContainingAll" "Android Health Connect audit"
...
assert_file_contains "$ANDROID_DIR/app/src/main/java/com/goose/android/MainActivity.java" "secondaryAction(\"Motion\"" "Android raw-motion step estimate button"
assert_file_contains "$ANDROID_DIR/app/src/main/java/com/goose/android/MainActivity.java" "raw_motion_step_estimator" "Android capture session raw-motion provenance"
assert_file_contains "$ANDROID_DIR/app/src/main/java/com/goose/android/MainActivity.java" "explicit_device_counter_status" "Android capture session explicit counter provenance"
assert_file_contains "$ANDROID_DIR/app/src/main/java/com/goose/android/MainActivity.java" "modeButton(\"Health\")" "Android report metric action layout"
...
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "\"operation_complete\"" "BLE session audit smoke"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "BLE session audit missing command-ready client hello completion row" "BLE session audit smoke"
assert_file_contains "$ANDROID_DIR/app/src/androidTest/java/com/goose/android/GooseRustBridgeInstrumentationTest.java" "BLE session audit missing ready hello command-ready row" "BLE session audit smoke"
...
assert_file_contains "$SCRIPT_DIR/android_port_status.sh" "Final PR evidence gate after prepared controlled capture:" "Android port status"
assert_file_contains "$SCRIPT_DIR/android_port_status.sh" "Scripts/android_final_pr_gate.sh tmp/android-phone-final-gate-real --skip-validate" "Android port status"
assert_file_contains "$SCRIPT_DIR/android_port_status.sh" "Lower-level diagnostic phone evidence gate:" "Android port status"
assert_file_contains "$SCRIPT_DIR/android_port_status.sh" "Scripts/android_partial_phone_gate.sh tmp/android-phone-partial-gate-real --skip-validate" "Android port status"
...
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "GOOSE_ANDROID_ADB_COMMAND_TIMEOUT_SECONDS" "database pull adb timeout"
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "Android adb devices" "database pull adb timeout"
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "Android adb state" "database pull adb timeout"
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "run_with_timeout" "database pull adb timeout"
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "run_to_file_with_timeout" "database pull adb timeout"
assert_file_contains "$SCRIPT_DIR/pull_android_database.sh" "timed out after" "database pull adb timeout"
